...
/* for SMIC - jhsong, 170427
 * add GetHash, SetHash, UpdateHash for finding duplicate path
 * modify serialize functions
 */

uint32_t
ContentObjectHeader::GetHash () const
{
  return m_hash;
}

void
ContentObjectHeader::SetHash (uint32_t seed)
{
  m_hash = seed;
}

uint32_t
ContentObjectHeader::UpdateHash (uint32_t nodeId)
{
...
{
  uint32_t size = 2 + ((2 + 2) + (m_name->GetSerializedSize ()) + (2 + 2 + 4 + 2 + 2 + (2 + 0))) + 4;
...
ContentObjectHeader::Serialize (Buffer::Iterator start) const
{
  ...
  start.WriteU32 (m_hash); // hash value
