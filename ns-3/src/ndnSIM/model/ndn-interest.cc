...
/* Modifier: J. Song
	Add m_branchInt(integer list) field to control interest-propagating-path
...
void
InterestHeader::SetPathId (int pathId)
{
  m_pathId = pathId;
}

int
InterestHeader::GetPathId () const
{
  return m_pathId;
}

// modified due to pathId, int = 1
uint32_t
...
InterestHeader::Serialize (Buffer::Iterator start) const
{
  ...
  start.WriteU8 (m_pathId); // path identifier
}

// modified due to branchInt
uint32_t
InterestHeader::Deserialize (Buffer::Iterator start)
{
  ...
  i.ReadU16 ();
  i.ReadU16 ();
  m_pathId = i.ReadU8 (); // path identifier
