 ...
 */
class ConsumerWindowMic: public Consumer
{
public: 
...
private:
  ...
  bool			m_snpCalled;
  double		m_windowFraction[MAX_PATHID];
  uint32_t		m_preWindow[MAX_PATHID];
  uint32_t		m_ssth[MAX_PATHID];
  uint32_t		m_l1[MAX_PATHID];	// # of bits between last two losses
  uint32_t		m_l2[MAX_PATHID];	// # of bits after last loss
  double		m_l[MAX_PATHID];	// fastest rate among time 1 & 2
  double		m_r1[MAX_PATHID];	// avg rtt of paths at time 1
  double		m_r2[MAX_PATHID];	// avg rtt of paths at time 2
  uint32_t		m_m[MAX_PATHID];	// branches that have largest windo
  uint32_t		m_b[MAX_PATHID];	// branches that show best thrput.
  uint32_t		m_nm;	// # of m
  uint32_t		m_nb;	// # of n
