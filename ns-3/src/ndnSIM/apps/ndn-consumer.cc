...
Consumer::SetRetxTimer (Time retxTimer)
{
  ...
  NS_LOG_DEBUG("RetxTimer is set:" << m_retxTimer << ", minRTO: " << m_rtt->GetMinRto() << ", maxRTO: " << m_rtt->GetMaxRto() << ", curRTO: " << m_rtt->RetransmitTimeout() << ", curRTT: " << m_rtt->GetCurrentEstimate());
...
Consumer::CheckRetxTimeout ()
{
  ...
  NS_LOG_DEBUG ("Checking Retx Timeout per " << m_retxTimer << ", rto: " << rto);
